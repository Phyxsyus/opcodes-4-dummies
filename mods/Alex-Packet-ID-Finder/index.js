...
module.exports = function AlexPacketIdFinder(mod) {
	...
	function printMainStatus() {
		if (enabled) {
			...
			protocol.maps.get(mod.dispatch.protocolVersion).name.set(filterExpression, 0)
		} else {
	...
	function isDefPerhapsWrong(name, packet, incoming, data, code) {
		...
		let protocolVersion = mod.dispatch.protocolVersion
	...
	function rebuildFilteredPacketDefList() {
		...
		for (let name of fullPacketDefList) {
			let code = protocol.maps.get(mod.dispatch.protocolVersion).name.get(name)
	...
	mod.hook('*', 'raw', { order: 999, type: 'all' }, (code, data, incoming, fake) => {
		if (!enabled) return
